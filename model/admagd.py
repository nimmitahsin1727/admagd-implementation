...
class ADMAGD(AuthorTopicModel):
    ...
    def calculate_phi_update(self):
        return (self.word_topic_matrix + self.beta) / (
            self.word_topic_matrix.sum(axis=1)[:, np.newaxis]
            + self.vocab_size * self.beta
        )

    def calculate_theta_update(self):
        return (self.author_topic_matrix + self.alpha) / (
            self.author_topic_matrix.sum(axis=1)[:, np.newaxis]
    ...
    def calculate_topic_probabilities(self, doc_id, word_id, author):
        ...
        author_probs = (
            author_topic_totals / sum(author_topic_totals)
            if sum(author_topic_totals) != 0
            else np.ones(self.num_topics) / self.num_topics
        )
    ...
    def gibbs_sampling(self, iterations=10):
        for iteration in range(iterations):
            ...
            for doc_id, document in enumerate(self.corpus):
                ...
                # Pre-compute some values that are reused
                author_idx = self.authors.index(author)
                ...
                for word_pos, (word_id, _) in enumerate(document):
                    ...
                    # Update matrices using vectorized operations if possible
